    ...
    return (
        <div className="h-screen bg-shnoor-lavender flex flex-col overflow-hidden">
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                ...
                {isCodingQuestion && (
                    <main className="flex-1 flex flex-col lg:flex-row bg-shnoor-lavender overflow-hidden coding-container relative">
                        {/* Mobile Tab Switcher */}
                        <div className="lg:hidden flex bg-white border-b border-gray-200 sticky top-0 z-20 flex-shrink-0 shadow-sm">
                            {['questions', 'description', 'code'].map(tab => (
                                <button
                                    key={tab}
                                    onClick={() => setActiveCodingTab(tab)}
                                    className={`flex-1 py-3 text-[11px] font-bold uppercase tracking-widest transition-all relative ${activeCodingTab === tab ? 'text-shnoor-indigo' : 'text-slate-400 hover:text-slate-600'
                                        }`}
                                >
                                    {tab === 'description' ? 'Description' : tab}
                                    {activeCodingTab === tab && (
                                        <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-shnoor-indigo" />
                                    )}
                                </button>
                            ))}
                        </div>
                        ...
                        <aside className={`w-52 bg-white border-r border-shnoor-mist flex-col flex-shrink-0 overflow-y-auto ${activeCodingTab === 'questions' ? 'flex w-full' : 'hidden lg:flex'} pb-16 lg:pb-0`}>
                        ...
                        <div
                            className={`bg-white border-r border-shnoor-light overflow-y-auto flex-shrink-0 shadow-lg ${activeCodingTab === 'description' ? 'flex flex-col flex-1' : 'hidden lg:block'} pb-16 lg:pb-0`}
                            style={{ 
                                width: window.innerWidth >= 1024 ? `${leftPanelWidth}%` : '100%', 
                                minWidth: window.innerWidth >= 1024 ? '18%' : '100%', 
                                maxWidth: window.innerWidth >= 1024 ? '45%' : '100%' 
                            }}
                        >
                            <div className="p-6">
                                {/* Title */}
                        ...
                        <div
                            className="hidden lg:block w-1 bg-shnoor-light hover:bg-shnoor-indigo cursor-col-resize flex-shrink-0 transition-colors duration-200" onMouseDown={handleHorizontalMouseDown}
                        ...
                        <div className={`flex-1 flex-col bg-shnoor-navy code-editor-container min-w-0 shadow-xl ${activeCodingTab === 'code' ? 'flex' : 'hidden lg:flex'} pb-16 lg:pb-0`}>
                            ...
                            {/* Vertical Resize Handle */}
                            <div
                                className="hidden lg:block h-1 bg-shnoor-indigo/30 hover:bg-shnoor-indigo cursor-row-resize flex-shrink-0 transition-colors duration-200" onMouseDown={handleVerticalMouseDown}
